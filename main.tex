...
\AtBeginSection[]{%
    { % <--- 注意这里加了一个大括号，限制修改的作用范围（局部作用域）
        % 将页脚左侧文字临时改为 "Outline"
        \renewcommand{\footlefttext}{Outline} 

        \begin{frame}
            \frametitle{Outline}
            \textbf{\tableofcontents[currentsection]} 
        \end{frame}
    } % <--- 作用域结束，之后的页面会自动恢复显示章节名
}
...
\title{Beamer for HZAU}
\subtitle{A Simple while elegant template（支持中文）}
\author[PP.Mei]{梅佩萍}
\mail{examPle@webmail.hzau.edu.cn}
\institute[HZAU]{College of Informatics \\
Huazhong Agricultural University}
\date{\today}
\titlegraphic[width=4cm]{attachments/hzau_logo.png}{}
...
\begin{columns}
	\column{0.5\textwidth}
		...
		\includegraphics[width=5cm]{attachments/hzau_logo.png}
		\caption{分栏示意图}
	\end{figure}
...
\begin{figure}[h]
    \centering
    \subfigure[校徽1]{
    \includegraphics[scale=0.2]{attachments/hzau_logo.png}
    }
    \subfigure[校徽2]{
    \includegraphics[scale=0.2]{attachments/hzau_logo.png}
    }
